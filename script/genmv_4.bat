...
:: 1. Redirection de la sortie vers un fichier texte
VBoxManage list vms > liste_temp.txt

:: 2. Parsing du fichier ligne par ligne
FOR /F %%a in (liste_temp.txt) do call :afficher_meta %%a

:: 3. Suppression du fichier temporaire après lecture
...
echo ----------------------------------------
echo Machine : %VM_EXT%
:: Récupération et affichage des métadonnées stockées
